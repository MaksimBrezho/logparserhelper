...
Откройте файл лога через меню **Commands**, выделите строки и запустите **Create Pattern**, чтобы построить новый шаблон. Используйте **Save Patterns** в том же меню для сохранения шаблонов или выберите **Code Generator** для создания конвертера CEF.
...
Готовый установщик `ALLtoCEF` появится в папке `dist` и будет использовать иконку из `icon/ALLtoCEF.png`. Скрипт также упакует эту иконку и необходимые файлы шаблонов и CEF.
